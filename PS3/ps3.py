...
def get_word_score(word: str, n):
    ...
    """
    word = word.lower()
    first_component_points = 0
    for letter in word:
        if letter == "*":
            continue
        first_component_points += SCRABBLE_LETTER_VALUES[letter]

    second_component_points = max(7 * len(word) - 3 * (n - len(word)), 1)

    return first_component_points * second_component_points
...
def deal_hand(n):
    ...
    for _ in range(num_vowels - NUM_WILD_CARDS):
...
#
def update_hand(hand: Dict[str, int], word):
    """
...
def is_valid_word(word: str, hand: Dict[str, int], word_list: List[str]) -> bool:
    ...
    for vowel in VOWELS.lower():
        candidates.append(word.replace("*", vowel))
...
def play_hand(hand: Dict[str, int], word_list: List[str]) -> int:
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        display_hand(hand)
        word = input('Enter or "!!" to indicate that you are finished:')
